...
import gmail.theultimatehose.ee.network.sync.IPacketSyncerToClient;
...
public class TileEntitySolder extends TileEntityInventoryBase implements IEnergyReceiver, IPacketSyncerToClient {
	...
	private boolean canDesolder() {
		if (slots[SLOT_PCB_IN]!=null) {
			...
			if (in.getItem().equals(Main.pcbLVBurnt)) {
				if (out == null) {
					return true;
				} else if (out.getItem().equals(Main.pcbScrapLV)) {
					if (out.stackSize+1 < out.getMaxStackSize()) {
						return true;
					}
				}
			} else if (in.getItem().equals(Main.pcbHVBurnt)) {
				if (out == null) {
					return true;
				} else if (out.getItem().equals(Main.pcbScrapHV)) {
					if (out.stackSize+1 < out.getMaxStackSize()) {
						return true;
					}
				}
	...
	private void finishDesolder() {
		...
		ItemStack out = slots[SLOT_PCB_OUT];
		if (in.getItem().equals(Main.pcbLVBurnt)) {
			if (out==null) slots[SLOT_PCB_OUT] = new ItemStack(Main.pcbScrapLV, 1);
			else if (out.getItem().equals(Main.pcbScrapLV)) slots[SLOT_PCB_OUT].stackSize++;
		} else if (in.getItem().equals(Main.pcbHVBurnt)) {
			if (out==null) slots[SLOT_PCB_OUT] = new ItemStack(Main.pcbScrapHV, 1);
			else if (out.getItem().equals(Main.pcbScrapHV)) slots[SLOT_PCB_OUT].stackSize++;
		}
    ...
    public void finishSolder() {
        ...
        if (slots[SLOT_CRAFT_RES].getItem() == Main.gaussGun) {
    ...
    @SideOnly(Side.CLIENT)
    public int getSolderTimeToScale(int i) { return this.currSolderTime * i / this.solderTime; }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
    	return slot == SLOT_PCB_IN && (stack.getItem()==Main.pcbHVBurnt || stack.getItem() == Main.pcbLVBurnt);
    }
