...
public class TileEntityMAD extends TileEntityInventoryBase implements IEnergyReceiver, IPacketSyncerToClient {
    ...
    private boolean canSort() {
        if (slots[SLOT_SCRAB_IN] != null) {
            if (slots[SLOT_SCRAB_IN].getItem() == Main.pcbScrapLV || slots[SLOT_SCRAB_IN].getItem() == Main.pcbScrapHV) {
                if (canSlotHandle(nextOut, 1)) {
                    return true;
                } else {
                    generateNextOutput(slots[SLOT_SCRAB_IN].getItem() == Main.pcbScrapHV);
                    return false;
                }
            }
        }
    ...
    private void finishEtch() {
        if (slots[SLOT_PCB_OUT] == null) {
            slots[SLOT_PCB_OUT] = new ItemStack(Main.pcbEtched);
        } else {
